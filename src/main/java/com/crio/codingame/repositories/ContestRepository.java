...
public class ContestRepository implements IContestRepository {
    ...
    public List<Contest> findAll() {
        ...
        for(String key:contestMap.keySet()){
            contestList.add(contestMap.get(key));
        }
     return contestList;
    }
    ...
    public List<Contest> findAllContestLevelWise(Level level) {
        List<Contest> contestLevelWise = new ArrayList<>();
        for (String key: contestMap.keySet()){ 
            if (contestMap.get(key).getLevel()==level)
            {
                contestLevelWise.add(contestMap.get(key));
            } 
